...
- **Global cycle hotkeys** (see Fast Screen Switcher): forward/backward per cycle group, cycling real game windows.
...
EVE-O Preview–style hotkey cycling between the REAL game windows. Config UI opened via the "⇄ CYCLE GROUPS" header button (a 920px modal):
- Up to 5 **cycle groups**. Each group has: forward hotkey(s), backward hotkey(s) (optional), and an **ordered member list** of characters.
- Pressing a group's forward/backward hotkey (global hook, e.g. F13/Control+F13) brings the NEXT/PREVIOUS group member's real EVE client window to foreground (SetForegroundWindow) — cycling wraps around; skips clients that aren't running.
- A character may belong to several groups.
- **Config UI**: per group, a roster of all detected characters as clickable chips — click to add/remove; order = click sequence, shown as a position number on the chip and as an "order:" sequence line. Hotkeys shown as blue badges (editable fields in the real app: click and press keys).
- **Card indicator**: each character card's header shows blue "G1·3"-style tags (group · position) so the cycle order is visible on the dashboard, mirroring EVE-O's cycle group indicator.
- **Persistence & compatibility**: settings stored in a JSON config; MUST support importing/exporting EVE-O Preview's config format — read `CycleGroupNForwardHotkeys`, `CycleGroupNBackwardHotkeys`, `CycleGroupNClientsOrder` (window-title keys like "EVE - Commander Tyrael", 1-based order values; equal values = tie, keep insertion order), and `PerClientAliases`. Match clients by window title.
- Styling: modal bg #10151b, border #3a4550; member chips border/tint #4dabf7 when selected, #232b35/#5c6875 when not; position numbers #4dabf7 600.
...
- Global: character order, active squad, cycle groups [{forwardHotkeys[], backwardHotkeys[], orderedMemberIds[]}], chat messages [{time, channel, sender, text, mergeCount}], channel filter, settings (thumbnails, alertFlash, voiceAlerts).
